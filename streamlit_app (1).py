...
all_months = sorted(snapshots.keys())
...
with st.sidebar:
    ...
    year_snaps = {m: df for m, df in snapshots.items() if m.startswith(sel_year + "-")}
    months = sorted(year_snaps.keys())
    st.markdown("---")
    st.markdown(f"**{sel_year}년 누적**: {months[0]} ~ {months[-1]} ({len(months)}개월)")
    st.markdown("**구분 색상**  \n🟩 신규  🟧 인상  🟦 인하  🟥 삭제")

mat, mon = company.build_matrix(maker, snapshots=year_snaps)
if not len(mat):
...
# 전체 매트릭스 + 다운로드
st.markdown(f"### 📊 {sel_year}년 전체 월별 매트릭스")
st.dataframe(style_matrix(mat, mon), width="stretch", height=460)
